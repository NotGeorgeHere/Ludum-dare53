...
sprite_index = spr_containerFour;
...
if(position_meeting(mouse_x, mouse_y, self)){
	...
	if (mouse_check_button_released(mb_left) && global.dragging_object && global.dragged_object == id){
		...
		if (place_meeting(x,y,obj_chuteRed)){
			global.totalScore += 1;
			instance_destroy(self);
			audio_play_sound(sound_success, 1, false);
		}
		else if (place_meeting(x,y,obj_chuteOrange) or place_meeting(x,y,obj_chutePurple) or place_meeting(x,y,obj_chuteBlue)){
			global.totalScore -= 1;
			instance_destroy(self);
	...
	if (global.dragging_object && global.dragged_object == id) {
		sprite_index = spr_containerMovingFour;
